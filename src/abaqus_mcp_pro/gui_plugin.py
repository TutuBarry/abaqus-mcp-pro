...
def _run_kernel_code(code, timeout):
    response_path = os.path.join(
        tempfile.gettempdir(), "abaqus_mcp_pro_%s.json" % uuid.uuid4().hex
    )
...
def start_gui_agent():
    ...
    if _SERVER is not None:
        ...
        return "Abaqus MCP Pro GUI agent is already running on %s:%s" % (HOST, PORT)
    ...
    return "Abaqus MCP Pro GUI agent listening on %s:%s" % (HOST, PORT)


def stop_gui_agent():
    global _SERVER
    if _SERVER is None:
        return "Abaqus MCP Pro GUI agent is not running."
    try:
        _SERVER.shutdown()
    ...
    return "Abaqus MCP Pro GUI agent stopped."
...
class McpGuiActionForm(AFXForm):
    ...
    def onCmdStart(self, sender, sel, ptr):
        try:
            ...
            if "already running" in message:
                ...
                _announce(banner)
                _announce("Abaqus MCP Pro GUI plugin log: %s" % LOG_PATH)
        except Exception as exc:
            message = "Abaqus MCP Pro GUI agent failed: %s" % exc
            _log(message)
    ...
    def onCmdStop(self, sender, sel, ptr):
        try:
            ...
            message = "Abaqus MCP Pro GUI agent stop failed: %s" % exc
            _log(message)
            showAFXErrorDialog(getAFXApp().getAFXMainWindow(), message)
        return 1
