...
def _resolve_mcp_home():
    ...
    try:
        ...
        pass
    return os.path.join(os.path.expanduser('~'), '.abaqus-mcp-pro')
...
def _start_worker(interval=0.1, mode_name='background'):
    global _mcp_running, _mcp_thread, _mcp_generation, _mcp_poll_interval, _mcp_generation
...
def mcp_loop(sleep_interval=0.1):
    ...
    print('     echo $null > "$env:USERPROFILE\\.abaqus-mcp-pro\\stop.flag"')
    print('')
